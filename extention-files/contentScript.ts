...
function extractLotNumber(): string {
  // Priority 1: New Copart structure - find "Lot number:" label and get next span with p-ml-1 class
  const lotNumberLabels = Array.from(document.querySelectorAll('span')).filter(
    span => span.textContent?.includes('Lot number:')
  );
  ...
  const lotNumberElement = document.querySelector('#LotNumber') as HTMLElement;
  if (lotNumberElement) {
...
function extractBidAmount(): string {
  ...
  const monsterBidInput = document.querySelector('#monster-bid-input') as HTMLInputElement;
...
function getActionTypeFromButton(button: HTMLElement): string {
  // Pay button
...
function getActionDescription(button: HTMLElement): string {
  if (button.classList.contains('cprt-btn-blue') || button.textContent?.includes('Pay')) {
